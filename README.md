...
| `pdf-editor-ocr-fast.html` | ~5.8 MB | fast | ⚠️ **Flagged — not compliant.** OCR assets are not inlined; see [`AUDIT_NOTE.md`](AUDIT_NOTE.md). | Do not rely on until rebuilt. |
| `pdf-editor-ocr-full.html` | ~16 MB | full | ⛔ Not yet in repo | You OCR messy scans and want accuracy. |

The lean build is the verified, recommended default. The OCR-fast build is
present in `public/` but is **flagged**: its OCR engine is configured to fetch
the worker, WASM core, and language model from a CDN at runtime, which violates
invariant 3. Under the CSP those fetches are *blocked* (so privacy holds), but
OCR is therefore broken, not working-and-inlined. It must be rebuilt with the
assets inlined before it can be advertised as functional. Details and remediation
in [`AUDIT_NOTE.md`](AUDIT_NOTE.md).
...
On the current `ocr-fast` build this is expected: its OCR assets are not inlined
and the CSP correctly blocks the CDN fetch. This is the flagged bug, not a
configuration issue on your end — see [`AUDIT_NOTE.md`](AUDIT_NOTE.md). The lean
